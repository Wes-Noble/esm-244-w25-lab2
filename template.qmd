...
f3   =  mass~bill_d+flip_l+sex+species
...
what to select the amount of folds (or how much training data) -- for this question we are told we want 10 folds
...
for loop: 
  apply the model into each training set
...
# replace puts the individual number taken out from the previous sample back into it for the next sample
...
for( i in 1:folds){
  ...
  kfold_rmse<-kfold_predict_test |> 
    summarize(rmse = calc_rmse(mass, model1))
  
  rmse_vec[i]<-kfold_rmse$rmse
}
